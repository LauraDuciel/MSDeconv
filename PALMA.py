...
def ent(x,a):
    ...
    xpos = x[x> 1e-8]/a           #if x = 0: rent sera infini (log (0)) ==> erreur!
...
####################################### PALMA algo  #######################################
      ##############################################################################
def _PPXAplus(K, Binv, y, eta, nbiter=1000, lamda=0.1, prec=1E-12, full_output=False):
    r"""
    ...
    x_n[x_n <0] = 0    #positivité imposée
    return x_n, comp


def PPXAplus(K, Binv, y, eta, nbiter=1000, lamda=0.1, prec=1E-12, full_output=False, ref=None):
    r"""
    performs the PPXA+ algorithm
    K : a MxN matrix which transform from data space to image space
    Binv : inverse of (Id + K.t K)
    ...
    for n in range(0,nbiter):
        if n%10 == 0: 
            sys.stdout.write("\r %d iterations are done "%n)  
        xx1 = prox_l1_Sent(tmp1, lamda, a)   # L1 + Shannon
        xx2 = prox_l2(tmp2, y, eta)
        c = np.dot(Binv, xx1 + np.dot(Kt,xx2))
        cmxn = c - x_n
        ...
        n_x_n = np.linalg.norm(x_n-x_n_old) / np.linalg.norm(x_n)
        if np.isnan( x_n.sum() ):  # Nan appear sometimes in pathological cases
            break
        if n_x_n < prec:
            break
        ...
        if full_output is True:
            ...
            lresidus.append(residus(x_n,K,y))
        if ref is not None: 
            refspec.append(-10*np.log10((np.linalg.norm(x_n.reshape(ref.shape)-ref)**2)/np.linalg.norm(ref)**2))   #    3 - eliminate scaling step
 
    x_n = x_n * scale
    x_n[x_n <0] = 0
    if full_output:
        comp = [lcrit, lent, lL1, lresidus]
        print ("\n Recovered signal have the size =" , x_n.shape)
...
def PPXAplussparse(K, Binv, y, eta, nbiter=1000, lamda=0.1, prec=1E-12, full_output=False, ref=None):
    r"""
    performs the PPXA+ algorithm
    K : a MxN matrix which transform from data space to image space
    Binv : inverse of (Id + K.t K)
    ...
    for n in range(0,nbiter):
        ...
        xx1 = prox_l1_Sent(tmp1, lamda, a)   # L1 + Shannon
        xx2 = prox_l2(tmp2, y, eta)
        c = Binv.dot(xx1 + Kt.dot(xx2))
        cmxn = c - x_n
        c2mxn = c + cmxn
        tmp1 += gamma*(c2mxn - xx1)
        tmp2 += gamma*(K.dot(c2mxn) - xx2)
        x_n += gamma*cmxn
        ...
        if full_output is True:
            ...
            lresidus.append(residus(x_n,K,y))
        if ref is not None: 
            refspec.append((np.linalg.norm(x_n.reshape(ref.shape)-ref))/np.linalg.norm(ref))   #    3 - eliminate scaling step
