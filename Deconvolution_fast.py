...
def memoize_clear():
    for f in (pattern,Samplez2D, Samplez2Dt):
        f.cache.clear()
    print("*** memoize cleared !!! ***")
...
def Cvlmz(Axe, x, z, width=0.0, irregular=False):
    ...
    while mz1 < Axe[-1]:     # step over windows
        ...
        mznext = Axe[j]
        icenter = (imz1+j)//2
        center = Axe[icenter]       # mass and location of mean pattern used for convolution

        if standard:
            if wstandard: print("*** standard ***"); wstandard=False
            dm,it = pattern(z*center)           # mean pattern
            ss = samplez(Axe, dm, it, z*center, z=z, width=width, irregular=irregular)
            ss = np.roll(ss,-icenter)    # move back mean pattern to origin
            y += conv(xx,ss)
        else:
            if wstandard: print("*** non-standard ***"); wstandard=False
...
###########################################################  2D
def SsceneP2D(Axe1, Axe2, Zaxis1, Zaxis2, n=20, show=False):
    """
    build a typical scene, with n random species
    """
    N1 = len(Axe1)
    ...
    j = np.random.randint(0, Z1*Z2, size=n)*N1*N2 +  np.random.randint(10, int(0.8*N1), size=n)*N2+np.random.randint(10, int(0.8*N2), size=n)
    ...
    if show:
        ...
        print ("\n y size=", y.shape)
    return x, y
def Cu_sceneP2D(Axe1, Axe2, Zaxis1, Zaxis2, n=20, show=False):
    """
    build a typical scene, with n random species
    """
    N1 = len(Axe1)
    ...
    Z1 = len(Zaxis1)
    Z2 = len(Zaxis2)
    x = cupy.zeros(Z1*N1*Z2*N2)
    j = cupy.random.randint(0, Z1*Z2, size=n)*N1*N2 +  cupy.random.randint(10, int(0.8*N1), size=n)*N2+cupy.random.randint(10, int(0.8*N2), size=n)
    inten = 80*abs(cupy.random.randn(n)) + 20
    x[j] = inten
    x = x.reshape((Z1,Z2,N1,N2))
    y = cupy.asarray(Cvlmultiz2D(Axe1, Axe2, Zaxis1, Zaxis2, x))
    if show:
        print ("\n x size=", x.shape)
        print ("\n y size=", y.shape)
    return x.get(), y.get()

def SCvlmultiz2D(AxeM1, AxeM2, Zaxis1, Zaxis2, x, width1=0.0, width2=0.0, irregular=False):
    N1 = len(AxeM1)
    N2 = len(AxeM2)
    Z1 = len(Zaxis1)
    Z2 = len(Zaxis2)
    ...
    return y
def Cu_Cvlmultiz2D(AxeM1, AxeM2, Zaxis1, Zaxis2, x, width1=0.0, width2=0.0, irregular=False):
    N1 = len(AxeM1)
    N2 = len(AxeM2)
    Z1 = len(Zaxis1)
    Z2 = len(Zaxis2)
...
def StCvlmultiz2D(AxeM1, AxeM2, Zaxis1, Zaxis2, y, width1=0.0, width2=0.0, irregular=False):
    N1 = len(AxeM1)
    N2 = len(AxeM2)
    Z1 = len(Zaxis1)
    Z2 = len(Zaxis2)
    ...
    return x
def Cu_tCvlmultiz2D(AxeM1, AxeM2, Zaxis1, Zaxis2, y, width1=0.0, width2=0.0, irregular=False):
    N1 = len(AxeM1)
    N2 = len(AxeM2)
    Z1 = len(Zaxis1)
    Z2 = len(Zaxis2)
    x = cupy.zeros((Z1,Z2,N1,N2))
    #yy = y.reshape((N1,N2))
    yy = y.reshape((N1,N2))
    for i, z1 in enumerate(Zaxis1):
        for j, z2 in enumerate(Zaxis2):
            x[i,j,:,:] += cupy.asarray(tCvlmz2D(AxeM1, AxeM2, yy, z1, z2, width1=width1, width2=width2, irregular=irregular))
    return x.get()
...
def SCvlmz2D(AxeM1, AxeM2, x, z1, z2, width1=0.0, width2=0.0, irregular=False):
    global wstandard
    N1 = len(AxeM1)
    N2 = len(AxeM2)
    nshift = 0
    ...
    while mz1 < AxeM1[-1]:
        ...
        while mz2 < AxeM2[-1]:     # step over windows
            ...
            if standard:
                ...
                SS = Samplez2D(center1, center2, z1=z1, z2=z2, width1=width1, width2=width2, irregular=irregular)
                y += halfconv2D(xx,SS)
        ...
        imz1 = j1
    return y
def Cu_Cvlmz2D(AxeM1, AxeM2, x, z1, z2, width1=0.0, width2=0.0, irregular=False):
    global wstandard
    N1 = len(AxeM1)
    N2 = len(AxeM2)
    nshift = 0
    ...
    while mz1 < AxeM1[-1]:
        ...
        while mz2 < AxeM2[-1]:     # step over windows
            ...
            j2 = find_nearest(AxeM2, mz2+step)
            center2 = (mz2+step/2)
            xx[imz1:j1,imz2:j2] = cupy.asarray(x[imz1:j1,imz2:j2])   # copy values in window
            ...
            if standard:
                ...
                dm,it = pattern(z2*center2)   # mean pattern over F2 step windows
                ss = samplez2D(AxeM1, AxeM2, dm, it, z1*center1, z2*center2, z1=z1, z2=z2, width1=width1, width2=width2, irregular=irregular)
                ss = cupy.roll(ss,-icenter2)    # move back mean pattern to origin
                ss = cupy.roll(ss,-icenter1,axis=0)    # move back mean pattern to origin
                y += conv2D(xx,ss)
            else:
                if wstandard: print("*** non-standard ***"); wstandard=False
                # AxeM1, AxeM2 should be global
...
def StCvlmz2D(AxeM1, AxeM2, y, z1, z2, width1=0.0, width2=0.0, irregular=False):
    N1 = len(AxeM1)
    N2 = len(AxeM2)
    nshift = 0
    mz1 = AxeM1[nshift]
    ...
    while mz1 < AxeM1[-1]:
        ...
        while mz2 < AxeM2[-1]:     # step over windows
            ...
            if standard:
                ...
                SS = Samplez2Dt(center1, center2, z1=z1, z2=z2, width1=width1, width2=width2, irregular=irregular)
                x += halfconv2D(yy,SS)
        ...
        imz1 = j1
    return x
def Cu_tCvlmz2D(AxeM1, AxeM2, y, z1, z2, width1=0.0, width2=0.0, irregular=False):
    N1 = len(AxeM1)
    N2 = len(AxeM2)
    nshift = 0
    mz1 = AxeM1[nshift]
    ...
    while mz1 < AxeM1[-1]:
        ...
        while mz2 < AxeM2[-1]:     # step over windows
            ...
            j2 = find_nearest(AxeM2, mz2+step)
            center2 = (mz2+step/2)
            yy[imz1:j1,imz2:j2] = cupy.asarray(y[imz1:j1,imz2:j2])   # copy values in window
...
def Ssamplez2D(axemass1,axemass2, deltamass, intensities, MassRef1, MassRef2, z1=1, z2=1, width1=0.0, width2=0.0, irregular=False, transposed=False):
    """
...
#    np.maximum(0, np.nan_to_num(spec))     # remove negatives and NaNs
    ...
    return spec
def Cu_samplez2D(axemass1,axemass2, deltamass, intensities, MassRef1, MassRef2, z1=1, z2=1, width1=0.0, width2=0.0, irregular=False, transposed=False):
    """
    realizes the 2D sampling of the isotopic pattern, described by deltamass, intensities,
    located at mass MassRef1, MassRef2
    on the axemass1, axemass2 mass axes.
    ...
    spwidth1 = (axemass1[-1]-offset1)*(nn1+1)/nn1
    spwidth2 = (axemass2[-1]-offset2)*(nn2+1)/nn2
    tax1 = cupy.hstack( [cupy.arange(nn1), -cupy.arange(nn1,0,-1)] )   /(2*spwidth1)
    tax2 = cupy.hstack( [cupy.arange(nn2), -cupy.arange(nn2,0,-1)] )   /(2*spwidth2)
    fid = cupy.zeros((len(tax1), len(tax2)))
    for mz,intens in zip(deltamass, intensities):
        if irregular:
            mcorr1 = mzcorr(axemass1, mz/z1+ MassRef1/z1)
            freq1 =  ( mcorr1  - offset1 )
        ...
        if freq1<=spwidth1 and freq1>0 and freq2<=spwidth2 and freq2>0:
            if intens>1E-5:
                for i in range(len(tax1)):
                    fid[i,:] += intens * cupy.cos(2*cupy.pi*freq2*tax2)*cupy.cos(2*cupy.pi*freq1*tax1[i])
    w1 = max(4*spwidth1/(nn1), width1)  # 4 pixel gaussian broadening is a minimum 
    w2 = max(4*spwidth2/(nn2), width2)  # 4 pixel gaussian broadening is a minimum 
    fid *= cupy.exp(-(tax2*w2)**2)
    fid = (fid.T*cupy.exp(-(tax1*w1)**2)).T
    # 2D FFT
    spec1 = cupy.fft.rfft(fid[:,0])[1:].real
    fn1 = len(spec1)
    spec2 = cupy.fft.rfft(fid[0,:])[1:].real
    fn2 = len(spec2)
    spec_t = cupy.zeros((len(tax1),fn2))
    spec = cupy.zeros((fn1,fn2))
    if transposed:
        for i in range(len(tax1)):
            spec_t[i,:] = cupy.fft.rfft(fid[i,:])[1:].real
        for i in range(fn2):
            spec[:,i] = cupy.fft.rfft(spec_t[:,i])[1:].real
    else:   # direct
        for i in range(len(tax1)):
            spec_t[i,:] = cupy.fft.rfft(fid[i,:])[:-1].real
        for i in range(fn2):
            spec[:,i] = cupy.fft.rfft(spec_t[:,i])[:-1].real
#    np.maximum(0, np.nan_to_num(spec))     # remove negatives and NaNs
    cupy.maximum(0, cupy.nan_to_num(spec))
    mspec = spec.max()
    spec[spec<(1E-3*mspec)] = 0.0   # clean small values
    ss = spec.sum()
    if ss >0.0:
...
def _SSamplez2D(axemass1, axemass2, center1, center2, z1, z2, width1, width2, irregular, transposed=False):
    ...
    return r
def _Cu_Samplez2D(axemass1, axemass2, center1, center2, z1, z2, width1, width2, irregular, transposed=False):
    """same as pattern() + samplez2D() + roll() but returns FFT of spectrum"""
    icenter1 = find_nearest(axemass1, center1)
    icenter2 = find_nearest(axemass2, center2)
    dm,it = pattern(z2*center2)           # mean pattern
    ss = samplez2D(axemass1,axemass2, dm, it, z1*center1, z2*center2, z1=z1, z2=z2, width1=width1, width2=width2, irregular=irregular, transposed=transposed)
    ssr = cupy.roll(ss,-icenter2)    # move back mean pattern to origin
    ssr = cupy.roll(ssr,-icenter1,axis=0)    # move back mean pattern to origin
    if transposed:
        r = cupy.fft.rfft(ssr[::-1,::-1].ravel() )
    else:
        r = cupy.fft.rfft(ssr.ravel() )
    return r
...
def Cu_conv2D(x,pat):
    res = cupy.fft.irfft( cupy.fft.rfft(x.ravel()) * cupy.fft.rfft(pat.ravel()) )
    return res.reshape(x.shape)

def Shalfconv2D(x,Pat):
    "performs the convolution, using FFT, but Pat is already FFTed"
    res = np.fft.irfft(np.fft.rfft(x.ravel()) * Pat)
    return res.reshape(x.shape)
def Cu_halfconv2D(x,Pat):
    "performs the convolution, using FFT, but Pat is already FFTed"
    res = cupy.fft.irfft(cupy.fft.rfft(x.ravel()) * Pat)
    return res.reshape(x.shape)

CUDA = False
def cu_select(cuda_select):
    global halfconv2D,tCvlmz2D,Cvlmz2D,conv2D,samplez2D,_Samplez2D,Cvlmultiz2D,tCvlmultiz2D,sceneP2D
    #global samplez2D
    if cuda_select:
        import cupy
        print("***Running on GPU***")
        halfconv2D = Shalfconv2D
        tCvlmz2D = StCvlmz2D
        Cvlmz2D = SCvlmz2D
        conv2D = Sconv2D
        samplez2D = Cu_samplez2D
        _Samplez2D = _SSamplez2D
        Cvlmultiz2D = SCvlmultiz2D
        tCvlmultiz2D = StCvlmultiz2D
        sceneP2D = SsceneP2D
...
#     if cuda_select:
#         print("***Running on GPU***")
#         # halfconv2D = Cu_halfconv2D
#         # tCvlmz2D = Cu_tCvlmz2D
#         # Cvlmz2D = Cu_Cvlmz2D
#         # conv2D = Cu_conv2D
#         samplez2D = Cu_samplez2D
#         # _Samplez2D = _Cu_Samplez2D
#         # Cvlmultiz2D = Cu_Cvlmultiz2D
#         # tCvlmultiz2D = Cu_tCvlmultiz2D
#         # sceneP2D = Cu_sceneP2D
...
def scenePm(K, n=10, show=False):
    ...
    Z = M/N
    x = np.zeros(M)
    j = np.random.randint(100, M-100, size=n)
    inten = 30 + 70*abs(np.random.randn(n))
    x[j] = inten
    y = np.dot(K,x)
    x = x.reshape(Z,N)
    if show:
...
def buildK(axemass, zaxis, width=0, irregular=False):
    ...
    for i,m in enumerate(axemass):
        ...
        for j,z in enumerate(zaxis):
            ...
            mat[:,i+N*j] += samplez(axemass, deltamass, intensities, m, width=width, z=z, irregular=irregular)
    return mat
